...
from django.contrib.auth.hashers import check_password
from .models import Account, Balance, Investment, InvestmentHistory,DailyProfitCredit
import logging
...
def login_view(request):
    if request.method == 'POST':
        ...
        user = None
        try:
            # Try authenticating with identifier as email
            ...
            if user is not None:
                ...
                # Check and credit daily profit
                today = timezone.now().date()
                active_investments = Investment.objects.filter(user=user, status='ACTIVE')
                
                for investment in active_investments:
                    with transaction.atomic():
                        # Check if user has been credited today for this investment
                        has_been_credited = DailyProfitCredit.objects.filter(
                            user=user,
                            investment=investment,
                            credited_date=today
                        ).exists()
                        
                        if not has_been_credited:
                            # Get or create user's balance
                            balance, created = Balance.objects.get_or_create(
                                user=user,
                                defaults={'investment_status': 'RUNNING'}
                            )
                            
                            # Update Total_Return and investment status
                            balance.Total_Return += investment.daily_profit
                            balance.investment_status = 'RUNNING'
                            balance.save()
                            
                            # Record the daily profit credit
                            DailyProfitCredit.objects.create(
                                user=user,
                                investment=investment,
